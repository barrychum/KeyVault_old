param ($SECRET, $LENGTH = 6, $WINDOW = 30)

$SECRET = $SECRET -replace ' ', ''

function Get-TimeByteArray($WINDOW) {
    $epoch = (New-TimeSpan -Start (Get-Date -Year 1970 -Month 1 -Day 1 -Hour 0 -Minute 0 -Second 0) -End (Get-Date).ToUniversalTime()).TotalSeconds
    $unixTime = [Convert]::ToInt64([Math]::Floor($epoch / $WINDOW))
